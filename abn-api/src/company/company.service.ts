...
export class CompanyService {
  ...
  search(q: SearchDto) {
    ...
    const sortKey = q.sort || "legal_name";
    const needle = (q.q || "").toLowerCase();

    let items = this.filterItems(this.data, q, needle);
    items = this.sortItems(items, sortKey, order);

    return this.paginate(items, page, pageSize);
  }

  private filterItems(data: any[], q: SearchDto, needle: string) {
  ...
  private sortItems(items: any[], sortKey: string, order: number) {
    return items.sort(
      (a, b) =>
        order *
        (a[sortKey] || "")
          .toString()
          .toLowerCase()
          .localeCompare((b[sortKey] || "").toString().toLowerCase())
    );
  }

  private paginate(items: any[], page: number, pageSize: number) {
